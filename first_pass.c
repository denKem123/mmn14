 ...
 * @param operand_one The first operand to check
 * @param operand_two The second operand to check
 */
static void build_extra_codeword_fpass(machine_word **code_img, long *ic, char *operand_one, char *operand_two);
...
static void build_extra_codeword_fpass(machine_word **code_img, long *ic, char *operand_one, char *operand_two)
{
	addressing_type first_operand_addressing = get_addressing_type(operand_one);
	addressing_type second_operand_addressing = get_addressing_type(operand_two);
	...
	if (first_operand_addressing == REGISTER_ADDR || second_operand_addressing == REGISTER_ADDR)
	{
		...
		char *ptr;
		machine_word *word_to_write;
		bool should_inc_ic = TRUE;
		long value;

		if (first_operand_addressing != REGISTER_ADDR)
		{
			should_inc_ic = first_operand_addressing != NONE_ADDR ? FALSE : TRUE;
			value = strtol(operand_two + 2, &ptr, 10);
		}
		else if (second_operand_addressing != REGISTER_ADDR)
		{
			value = strtol(operand_one + 2, &ptr, 10) << 5;
		}
		else
		{
			value = strtol(operand_one + 2, &ptr, 10) << 5 || strtol(operand_two + 2, &ptr, 10);
		}

		if (should_inc_ic == TRUE)
		{
			(*ic)++;
			op_ic = (*ic);
		}
		...
		code_img[op_ic - IC_INIT_VALUE] = word_to_write;
	}
	if (first_operand_addressing != NONE_ADDR && first_operand_addressing != REGISTER_ADDR)
	{
		(*ic)++;
		if (first_operand_addressing == IMMEDIATE_ADDR)
		{

			char *ptr;
			machine_word *word_to_write;
			/* Get value of immediate addressed operand. notice that it starts with #, so we're skipping the # in the call to strtol */
			long value = strtol(operand_one, &ptr, 10);
			word_to_write = (machine_word *)malloc_with_check(sizeof(machine_word));
			word_to_write->length = 0; /* Not Code word! */
	...
	if (second_operand_addressing != NONE_ADDR && second_operand_addressing != REGISTER_ADDR)
	{
		(*ic)++;
		if (second_operand_addressing == IMMEDIATE_ADDR)
		{

			char *ptr;
			machine_word *word_to_write;
			/* Get value of immediate addressed operand. notice that it starts with #, so we're skipping the # in the call to strtol */
			long value = strtol(operand_two, &ptr, 10);
			word_to_write = (machine_word *)malloc_with_check(sizeof(machine_word));
			word_to_write->length = 0; /* Not Code word! */
			(word_to_write->word).data = build_data_word(IMMEDIATE_ADDR, value, FALSE);
