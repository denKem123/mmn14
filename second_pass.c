...
int process_spass_operand(line_info line, long *curr_ic, long *ic, char *operand_one, char *operand_two, machine_word **code_img,
						  table *symbol_table);

bool process_spass_operand_direct(line_info line, long *curr_ic, char *operand, addressing_type addr, machine_word **code_img,
								  table *symbol_table);
...
bool add_symbols_to_code(line_info line, long *ic, machine_word **code_img, table *symbol_table)
{
	...
	if (length > 1)
	{
		...
		if (operand_count--)
		{
			isvalid = process_spass_operand(line, &curr_ic, ic, operands[0], operands[1], code_img, symbol_table);
			free(operands[0]);
			if (operand_count)
			{
				free(operands[1]);
...
int process_spass_operand(line_info line, long *curr_ic, long *ic, char *operand_one, char *operand_two, machine_word **code_img,
						  table *symbol_table)
{
	addressing_type addr_one = get_addressing_type(operand_one);
	addressing_type addr_two = get_addressing_type(operand_two);
	/* if the word on *IC has the immediately addressed value (done in first pass), go to next cell (increase ic) */
	if (addr_one == IMMEDIATE_ADDR || addr_one == REGISTER_ADDR)
		(*curr_ic)++;
	if (DIRECT_ADDR == addr_one)
	{
		if (process_spass_operand_direct(line, curr_ic, operand_one, addr_one, code_img, symbol_table) == FALSE)
			return FALSE;
	}
	if (addr_two == IMMEDIATE_ADDR || (addr_two == REGISTER_ADDR && addr_one != REGISTER_ADDR))
		(*curr_ic)++;
	if (DIRECT_ADDR == addr_two)
	{
		if (process_spass_operand_direct(line, curr_ic, operand_two, addr_two, code_img, symbol_table) == FALSE)
			return FALSE;
	}
	return TRUE;
}
